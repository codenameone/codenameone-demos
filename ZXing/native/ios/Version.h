#ifndef __DATA_VERSION_H__
#define __DATA_VERSION_H__
...
private:
  ...
  ECBlocks* ecBlocks_;
  int totalCodewords_;
  VersionD(int versionNumber, int symbolSizeRows, int symbolSizeColumns, int dataRegionSizeRows,
		  int dataRegionSizeColumns, ECBlocks *ecBlocks);

public:
  static std::vector<Ref<VersionD> > VERSIONS;
  ...
  Ref<VersionD> getVersionForDimensions(int numRows, int numColumns);
    ~VersionD();
private:
  VersionD(const VersionD&);
  VersionD & operator=(const VersionD&);
};
}
}

#endif // __DATA_VERSION_H__
